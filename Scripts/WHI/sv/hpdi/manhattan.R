...
res1 = get(load("/home/jdominguez1/B64_DIAMETR/Scripts/WHI/sv/hpdi/model_2sva_hpdi.RData"))
...
p <- ggplot(don, aes(x=BPcum, y=-log10(P))) +
  ...
  geom_label_repel( data=subset(don, P<0.00001), aes(label=SNP), size=3,min.segment.length = 0,
                    fill = alpha(c("white"),0.5),max.overlaps = 20) +
  ...
  xlab("Chromosome") +
  labs(title = "Manhattan Plot WHI hpdi") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))

 ggsave(plot = p, filename = "~/B64_DIAMETR/Scripts/WHI/sv/hpdi/manhattanplot.png")  
 save(manh1, file="~/B64_DIAMETR/Scripts/WHI/sv/hpdi/model_2sva_noxy_hpdi.RData")
