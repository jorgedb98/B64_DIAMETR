...
res1 = get(load("/home/jdominguez1/B64_DIAMETR/Scripts/WHI/sv_bmi_pa_airwave/hpdi/bacon.RData"))
...
p <- ggplot(don, aes(x=BPcum, y=-log10(P))) +
  ...
  theme( 
   ...
   xlab("Chromosome") +
   labs(title = "Manhattan Plot WHI hpdi", subtitle = "Corrected") +
   theme(plot.title = element_text(hjust = 0.5, face = "bold"), plot.subtitle = element_text(hjust = 0.5, face = "italic"))

 ggsave(plot = p, filename = "~/B64_DIAMETR/Scripts/WHI/sv_bmi_pa_airwave/hpdi/manhattanplot_bacon.png")  
 save(manh1, file="~/B64_DIAMETR/Scripts/WHI/sv_bmi_pa_airwave/hpdi/bacon_noxy.RData")
