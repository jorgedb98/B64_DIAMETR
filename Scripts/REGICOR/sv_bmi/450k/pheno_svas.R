...
print(out.file)
surrogates = commandArgs()[13]
print(as.numeric(surrogates))
...
{
  mt_val <- mt_val[which(rownames(mt_val)%in%unique(pheno$sample_name)==T),]
...
pheno <- pheno[which(rownames(mt_val)%in%pheno$sample_name),]
