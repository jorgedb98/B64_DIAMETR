...
date()
score = commandArgs()[6]
dades.file = commandArgs()[7]
dades = get(load(dades.file))
...
chisq <- qchisq(1-dades[,7],1)
lambda = median(chisq)/qchisq(0.5,1)
png(paste("/home/jdominguez1/B64_DIAMETR/Scripts/REGICOR/sv/epic/dashf/qq_REGepic_CORRECTED_",
          score,".png",sep=""),height=3600,width=6000,res=600,units = "px")
qq(dades[,7])
lambdavalue<-lambda
text(1,4,paste0("lambda = ", round(lambdavalue,3)))
title(main="Q-Q plot REGICOR epic dashf CORRECTED")
dev.off()
