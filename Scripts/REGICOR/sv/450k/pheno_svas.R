...
pheno <- get(pheno)
str(pheno)
pheno <- distinct(pheno,sample_name, .keep_all= TRUE)
...
{
  # inter <- intersect(rownames(mt_val), pheno$sample_name)
  ...
  # rows <- rownames(mt_val)
  # save(rows, file ="/home/jdominguez1/rows.RData")
  # save(samples, file ="/home/jdominguez1/samples.RData")
  # print(dim(pheno))
  # print(length(rownames(mt_val)))
  # print(length(pheno$sample_name))
  # dif <- setdiff(rownames(mt_val), pheno$sample_name)
  # print(dif)
  mt_val <- mt_val[pheno$sample_name,]
  identical(pheno$sample_name, rownames(mt_val))
}
...
if (!is.null(chr_covariates)) {for (covi in chr_covariates) mod = paste(mod, "+", "as.factor(", covi, ")") }
mod <- paste(mod, ",data=pheno)", sep="")

mod0 <- NULL
if (!is.null(num_covariates)) {for (covi in num_covariates) mod0 = paste(mod0, "+", covi) }
if (!is.null(chr_covariates)) {for (covi in chr_covariates) mod0 = paste(mod0, "+", "as.factor(", covi, ")") }
